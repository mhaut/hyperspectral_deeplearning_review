...
def main():
	...
	for pos in range(args.repeat):
		...
		if args.use_val:
			...
			x_val   = np.expand_dims(x_val, axis=len(x_train.shape)+1)
		x_test   = np.expand_dims(x_test, axis=len(x_train.shape)+1)
		...
		clf.fit(x_train, keras_to_categorical(y_train, num_class),
						...
						callbacks = [ModelCheckpoint("/tmp/best_model.h5", monitor='val_acc', verbose=0, save_best_only=True)])
